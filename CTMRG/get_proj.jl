...
function get_QuL(ipeps::iPEPS, envs::iPEPSenv, x::Int, y::Int)
    ...
    M = ipeps[x, y]
    Mbar = bar(M)
    gate1 = swap_gate(space(Tr)[4], space(Tb)[3])
    gate2 = swap_gate(space(M)[5], space(M)[4])
    ...
    @tensor CTrTbM[rχ, bχ, rupMD, bupMD, rupD, rdnD, p] :=
        (CTrTb[rχ, bupDin, bdnDin, rupDin, rdnD, bχ] * gate1[rupD, lupMDin, bdnDin, rupDin]) *
        M[lupMDin, bupDin, p, rupMD, bupMD]
    @tensor CTrTbMMbar[(); (rχ, rupMD, rdnMD, bχ, bupMD, bdnMD)] :=
        (CTrTbM[rχ, bχ, rupMD, bupMDin, rupD, rdnD, p] * Mbar[rdnD, rupD, p, rdnMDin, bdnMD]) *
        gate2[bupMD, rdnMDin, bupMDin, rdnMD]
    return CTrTbMMbar
end
...
function get_QuR(ipeps::iPEPS, envs::iPEPSenv, x::Int, y::Int)
    ...
    M = ipeps[x, y]
    Mbar = bar(M)
    gate1 = swap_gate(space(Mbar)[1], space(Mbar)[2])
    gate2 = swap_gate(space(Mbar)[5], space(Mbar)[4])
    ...
    @tensor CTlTbM[lχ, lupD, ldnD, bχ, bupD, bdnD, p] :=
        (CTlTb[lχ, lupDin, ldnD, bχ, bupDin, bdnDin] * M[lupMDin, bupDin, p, lupDin, bupD]) *
        gate1[lupMDin, bdnDin, lupD, bdnD]
    @tensor CTlTbMMbar[lχ, lupD, ldnD; bχ, bupD, bdnD] :=
        (CTlTbM[lχ, lupD, ldnDin, bχ, bupDin, bdnDin, p] * gate2[bupDin, ldnDin, bupD, ldnMDin]) *
        Mbar[ldnD, bdnDin, p, ldnMDin, bdnD]
    return CTlTbMMbar
end
...
function get_QdL(ipeps::iPEPS, envs::iPEPSenv, x::Int, y::Int)
    ...
    Mbar = bar(M)
    gate1 = swap_gate(space(Mbar)[1], space(Mbar)[2])
    gate2 = swap_gate(space(M)[5], space(Mbar)[4])
    ...
    @tensor CTlTbMbar[tχ, rupD, rdnD, rχ, tupD, tdnD, p] :=
        (CTlTb[tχ, rupD, rdnDin, rχ, tupDin, tdnDin] * Mbar[rdnDin, tdnD, p, rdnMDin, tdnDin]) *
        gate2[tupDin, rdnD, tupD, rdnMDin]
    @tensor CTlTbMbarM[tχ, tupD, tdnD; rχ, rupD, rdnD] :=
        (CTlTbMbar[tχ, rupDin, rdnD, rχ, tupDin, tdnDin, p] * gate1[rupMDin, tdnD, rupDin, tdnDin]) *
        M[rupMDin, tupD, p, rupD, tupDin]
    return CTlTbMbarM
end
...
function get_QdR(ipeps::iPEPS, envs::iPEPSenv, x::Int, y::Int)
    ...
    M = ipeps[x, y]
    Mbar = bar(M)
    gate1 = swap_gate(space(M)[1], space(Mbar)[2])
    gate2 = swap_gate(space(Mbar)[4], space(M)[5])
    ...
    @tensor CTrTbMbar[tχ, lupD, ldnD, lχ, tupD, tdnD, p] :=
        (CTrTb[tχ, lupD, ldnDin, lχ, tupDin, tdnDin] * gate2[ldnDin, tupDin, ldnMDin, tupD]) *
        Mbar[ldnD, tdnD, p, ldnMDin, tdnDin]
    @tensor CTrTbMbarM[(lχ, lupD, ldnD, tχ, tupD, tdnD); ()] :=
        (CTrTbMbar[tχ, lupMDin, ldnD, lχ, tupDin, tdnDin, p] * M[lupDin, tupD, p, lupMDin, tupDin]) *
        gate1[lupD, tdnD, lupDin, tdnDin]
    return CTrTbMbarM
end