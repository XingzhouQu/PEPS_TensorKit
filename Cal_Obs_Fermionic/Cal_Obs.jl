...
function _2siteObs_adjSite(ipeps::iPEPS, ipepsbar::iPEPS, envs::iPEPSenv, Gates::Vector{String}, para::Dict{Symbol,Any},
    ...
    if y2 == y1  # 横向的两个点.
        ...
        @tensor leftpart[pup1, Dupin, t12t2; pdn1, Ddnin, b12b2] :=
        ...
        @tensor rightpart[pup2, t12t2, Dupin; pdn2, Ddnin, b12b2] :=
    ...
    elseif x2 == x1  # 纵向的两个点
        ...
        @tensor toppart[pup1, Dupin, l12l2; pdn1, Ddnin, r12r2] :=
        ...
        @tensor bottompart[pup2, Dupin, l12l2; pdn2, Ddnin, r12r2] :=
...
function _2siteObs_diagSite(ipeps::iPEPS, ipepsbar::iPEPS, envs::iPEPSenv, Gates::Vector{String}, para::Dict{Symbol,Any},
    ...
    if x1 == (x2 - 1 - Int(ceil((x2 - 1) / Lx) - 1) * Lx)  # 左上到右下的两个点. 这里调用 CTMRG 求环境的函数
        ...
        @strided @tensor QuL[(pup1); (pdn1, rχ, rupMD, rdnMD, bχ, bupMD, bdnMD)] :=
        ...
        @strided @tensor QdR[(lχ, lupD, ldnD, tχ, tupD, tdnD, pup4); (pdn4)] :=
    ...
    elseif x1 == (x2 + 1 - Int(ceil((x2 + 1) / Lx) - 1) * Lx)  # 右上到左下的两个点.  这里调用 CTMRG 求环境的函数
        ...
        @strided @tensor QuR[pup2, pdn2, lχ, lupD, ldnD; bχ, bupD, bdnD] :=
        ...
        @strided @tensor QdL[pup3, pdn3, tχ, tupD, tdnD; rχ, rupD, rdnD] :=
